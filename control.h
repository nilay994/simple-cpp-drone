...
inline uint16_t remap_throttle_signals(float val, uint16_t min, uint16_t max) {
	uint16_t tmpval = round((val - 0.5) * ((float)max - (float)min));
	if (tmpval > max) {
		tmpval = max;
	}
	if (tmpval < min) {
...
/* remap attitude signals, -0.5 to 0.5 is mapped to 1000 to 2000 
...
inline uint16_t remap_attitude_signals(float val, uint16_t min, uint16_t max) {
	uint16_t tmpval = round((val) * ((float)max - (float)min));
	if (tmpval > max) {
		tmpval = max;
	}
	if (tmpval < min) {
